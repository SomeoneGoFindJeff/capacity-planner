...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Clear filters whenever you navigate away
  ...
  if (tbl) {
    ...
    tbl.querySelector('tbody').addEventListener('click', ev => {
      ...
      if (ev.target.closest('.edit-btn')) {
        ...
        const typeOptions = JSON.parse(tbl.dataset.types).map(t=>`<option value="${t.id}" ${t.id==curType?'selected':''}>${t.name}</option>`).join('');
        typeCell.innerHTML = `<select class="input input-small edit-type">${typeOptions}</select>`;

        const groupCell  = tr.querySelector('.group-cell');
        const curGroup   = tr.dataset.groupId || '';
        const groupOptions= JSON.parse(tbl.dataset.groups).map(g=>`<option value="${g.id}" ${g.id==curGroup?'selected':''}>${g.name}</option>`).join('');
        groupCell.innerHTML = `<select class="input input-small edit-group">${groupOptions}</select>`;
